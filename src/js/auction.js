...
async function init() {
    ...
    $.getJSON('MyAuction.json', async contractABI => {
        ...
        try {
            ...
            const myProduct = await auction.getProductInfo(); // <--- getInfo | getProductInfo
            ...
            auction.BidEvent()
                .on('data', async event => {
                    await updateStatus(event, "BIDED");
                    await updateInfo();
            ...
            auction.EndEvent()
                .on('data', async event => {
                    ...
                    const highestBidder = event.returnValues.highestBidder;
                    const highestBid = event.returnValues.highestBid.call();
...
async function updateStatus(event, message) {
    const dateString = new Date(event .returnValues.timestamp * 1000); // convert second to millisecond => date
    $('#EventsLog').append('<li class="lead">' + message + ' at ' + dateString + '</li>' );
    const currentState = await auction.STATE.call();
    $('#State').text(AuctionState[currentState]);
}
...
async function updateAccounts() {
    ...
    $('#AccountList').on('change', async e => {
        ...
        const myBid = await auction.getMyBid.call(bidder);
...
const endAuction = async () => {
    let result;
    try {
        result = await auction.endAuction({from: auctionOwner});
        console.log('tx result', result);
    } catch (err) {
        console.log(err);
    }
